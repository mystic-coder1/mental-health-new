 ...
 * 
 * Usage:
 * - For students: <MentalHealthApp userRole="student" />
 * - For consultants: <MentalHealthApp userRole="consultant" />
 * 
 * Features:
 * - Role-based access control
 * - Database storage of assessment results
 * - Students see completion message only
 * - Consultants see full scores and analysis
 */

const MentalHealthApp = ({ userRole = 'student' }) => { // Default to student role
  ...
  const downloadReport = () => {
    if (userRole === 'consultant' && results) {
      // Detailed consultant report with scores
      const resultText = `
Mental Health Assessment - Professional Analysis Report
=====================================================
Student: ${userName || 'Anonymous User'}
...
OVERALL ASSESSMENT
==================
Risk Level: ${results.overallRisk.level}
Total Score: ${results.totalScore}/${results.maxPossibleScore} (${Math.round((results.totalScore / results.maxPossibleScore) * 100)}%)
Professional Recommendation: ${results.overallRisk.message}
...
${selectedCategories.map(categoryId => {
  ...
  return `${category.name}: ${levelInfo.level} - Score: ${score}/12 (${Math.round((score/12)*100)}%)
   Clinical Notes: ${levelInfo.message}`;
}).join('\n\n')}
...
${selectedCategories.map(categoryId => {
  ...
  return `\n${category.name.toUpperCase()} ASSESSMENT:\n${'-'.repeat(category.name.length + 12)}\n${questions[categoryId].map((q, index) => {
    ...
    return `Q${index + 1}: ${q.question}\nResponse: ${selectedOption?.text || 'Not answered'} (Score: ${selectedPoints || 0}/3)`;
  }).join('\n\n')}`;
}).join('\n\n')}

PROFESSIONAL NOTES
==================
This assessment provides a structured evaluation of the student's mental health status across multiple domains. 
The scoring system uses validated assessment criteria with scores ranging from 0-3 per question.

Interpretation Guidelines:
- Low Risk (0-25%): Minimal symptoms, routine monitoring recommended
- Moderate Risk (26-50%): Some symptoms present, regular check-ins advised  
- High Risk (51-75%): Significant symptoms, active intervention recommended
- Critical Risk (76-100%): Severe symptoms, immediate professional attention required

DISCLAIMER: This assessment is a screening tool and does not constitute a clinical diagnosis. 
Professional clinical judgment should always supersede automated scoring systems.

Report generated by Mental Health Assessment System
Confidential - For authorized personnel only
      ...
      link.href = url;
      link.download = `mental-health-assessment-professional-${new Date().toISOString().split('T')[0]}.txt`;
      document.body.appendChild(link);
      link.click();
    } else {
      // Student completion certificate
      const completionText = `
Mental Health Assessment - Completion Certificate
================================================
Student: ${userName || 'Anonymous User'}
Assessment ID: ${assessmentId || 'N/A'}
...
ASSESSMENT SUMMARY
==================
Assessment Categories Completed: ${selectedCategories.length}
...
NEXT STEPS
==========
Your assessment has been submitted and will be reviewed by qualified mental health professionals.
You will be contacted with follow-up recommendations and support resources.
...
====================
- This assessment is confidential and secure
- Results will only be shared with authorized mental health professionals
- This screening does not replace professional medical advice
- Follow-up contact will be initiated within 1-2 business days

Thank you for taking this important step toward your mental wellness.

Certificate generated by Mental Health Assessment System
      `.trim();
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `mental-health-assessment-completion-${new Date().toISOString().split('T')[0]}.txt`;
      document.body.appendChild(link);
      link.click();
    }
  ...
  const storeAssessmentInDatabase = async (assessmentData) => {
    ...
    try {
      ...
      localStorage.setItem('lastAssessmentId', mockId);
      localStorage.setItem(`assessment_${mockId}`, JSON.stringify(assessmentData));
      return { assessmentId: mockId };
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleAnswerSelect = async (points) => {
    ...
    if (currentQuestionIndex < allQuestions.length - 1) {
      ...
      // Store in database
      await storeAssessmentInDatabase(assessmentData);
      
      // Only set results for consultants, students get completion message only
  ...
  if (currentStep === 'results') {
    // Student View - No scores, just completion message
    if (userRole === 'student') {
      return (
        <div className="min-h-screen bg-gradient-to-br from-white to-gray-50">
          <div className="container mx-auto px-4 py-6 md:py-12">
            <div className="max-w-4xl mx-auto">
              ...
              <div className="text-center mb-8 md:mb-12">
                ...
                <h1 className="text-2xl md:text-4xl font-bold text-gray-900 mb-4">Assessment Submitted Successfully!</h1>
                <p className="text-base md:text-lg text-gray-600 mb-4">
                  {userName && `Thank you ${userName}! `}Your mental health assessment has been completed and securely stored.
                </p>
                ...
                {assessmentId && (
                  <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg mb-6">
                    ...
                    <p className="text-xs text-blue-600 mt-1">
                      Please save this ID for your records. Your counselor will review the results.
                    </p>
                  </div>
                )}
                {isSubmitting && (
              ...
              {/* What happens next */}
              <div className="bg-white p-6 md:p-8 rounded-xl shadow-lg mb-6 md:mb-8 border border-gray-200">
                <h2 className="text-xl md:text-2xl font-bold mb-4 text-gray-900">What Happens Next?</h2>
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600 text-sm">A qualified mental health professional will review your responses.</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600 text-sm">You'll receive tailored support and resource recommendations.</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600 text-sm">Our team will reach out with next steps and support options.</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-purple-50 to-blue-50 p-6 md:p-8 rounded-xl shadow-lg mb-6 md:mb-8 border border-purple-200">
                <div className="text-center">
                  <h3 className="text-lg md:text-xl font-semibold text-gray-900 mb-4">Need Immediate Support?</h3>
                  <p className="text-base md:text-lg text-gray-700 leading-relaxed mb-4">
                    If you're experiencing a mental health crisis or need immediate support, please don't wait for the assessment review.
                  </p>
                  <div className="grid md:grid-cols-2 gap-4 text-sm">
                    <div className="bg-white p-4 rounded-lg">
                      <p className="font-medium text-gray-900">Crisis Hotline:</p>
                      <p className="text-blue-600 font-semibold">988 (USA)</p>
                    </div>
                    <div className="bg-white p-4 rounded-lg">
                      <p className="font-medium text-gray-900">Campus Counseling:</p>
                      <p className="text-blue-600 font-semibold">Contact Student Health Center</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                <button
                  ...
                  className="px-6 py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition-all duration-300 flex items-center justify-center gap-2"
                >
                  <span>📄</span>
                ...
                <button
                  ...
                  className="px-6 py-3 bg-white border-2 border-purple-600 text-purple-600 rounded-lg font-semibold hover:bg-purple-50 transition-all duration-300 flex items-center justify-center gap-2"
                >
                ...
                <button
                  onClick={() => {
                    const mailtoLink = `mailto:counselor@university.edu?subject=Mental Health Assessment Completed&body=I have completed my mental health assessment (ID: ${assessmentId}). Please review when convenient.`;
                    window.location.href = mailtoLink;
                  }}
                  className="px-6 py-3 bg-purple-600 text-white rounded-lg font-semibold hover:bg-purple-700 transition-all duration-300 flex items-center justify-center gap-2"
                >
                  <span>📧</span>
                  Contact Counselor
                </button>
              ...
              <div className="text-center">
                <p className="text-sm text-gray-600 max-w-2xl mx-auto">
                  Your responses are confidential and will only be shared with authorized mental health professionals. 
                  This assessment is for support purposes and does not constitute a medical diagnosis.
                </p>
              </div>
            </div>
          </div>
    ...
    // Consultant View - Full results with scores
    return (
      <div className="min-h-screen bg-gradient-to-br from-white to-gray-50">
        <div className="container mx-auto px-4 py-6 md:py-12">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="text-center mb-8 md:mb-12">
              ...
              <h1 className="text-2xl md:text-4xl font-bold text-gray-900 mb-4">Assessment Analysis - Consultant View</h1>
              <p className="text-base md:text-lg text-gray-600 mb-4">
                {userName && `Student: ${userName} | `}Detailed assessment results and scoring analysis
              </p>
            ...
            {/* Overall Risk Assessment - Consultant View */}
            {results && (
              <>
                    ...
                    {results.overallRisk.level === 'Critical Risk' && (
                      <div className="mt-4 p-4 bg-red-100 border border-red-300 rounded-lg">
                        ...
                        <p className="text-sm mt-2 text-red-700">
                          Recommend immediate intervention and professional support.
                        </p>
                      </div>
                    )}
                  </div>
                ...
                {/* Category Results with Recommendations - Consultant View */}
                ...
                {/* Crisis Resources - Consultant View */}
                {(results.overallRisk.level === 'Critical Risk' || results.overallRisk.level === 'High Risk') && (
                  <div className="bg-red-50 border border-red-200 p-6 rounded-xl mb-8">
                    <h3 className="text-lg font-semibold text-red-800 mb-4 flex items-center gap-2">
                      <AlertCircle className="w-5 h-5" />
                      Crisis Resources & Immediate Actions
                    </h3>
                    <div className="grid md:grid-cols-2 gap-4 text-sm">
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-red-700">Contact student health center immediately</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
                  ...
                  <button
                    onClick={() => {
                      const mailtoLink = `mailto:student@university.edu?subject=Mental Health Assessment Results&body=Dear ${userName}, Your mental health assessment has been reviewed. Please schedule a follow-up appointment to discuss next steps.`;
                      window.location.href = mailtoLink;
                    }}
                    className="px-4 py-3 bg-orange-600 text-white rounded-lg font-semibold hover:bg-orange-700 transition-all duration-300 flex items-center justify-center gap-2"
                  >
                    <span>📧</span>
                    Email Student
                  </button>
                ...
                <div className="text-center">
                  <p className="text-sm text-gray-600 max-w-2xl mx-auto">
                    This assessment analysis is confidential and should only be shared with authorized personnel. 
                    Use professional judgment when interpreting results and making recommendations.
                  </p>
                </div>
              </>
            )}
